...
def test_list_presets(runner, mock_db):
    ...
    assert result.exit_code == 0
    assert "Available presets:" in simplified_output or "No presets found" in simplified_output
...
def test_invalid_option(runner):
    result = runner.invoke(invoke_presets_cli, ["database", "list-snapshots", "--invalid-option"])
    simplified_output = simplify_rich_output(result.stdout)
    assert result.exit_code != 0
    assert "root database list-snapshots [OPTIONS] Try 'root database list-snapshots" in simplified_output